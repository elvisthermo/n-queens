    ...
    */
    for (x = 0; x < n; x++) {
        ...
        for (y = 0; y < n; y++) {
            ...
            heuristicTable[y][x] = HeuristicCost()
            board[y][x] = false
        }
...
function HeuristicCost() {
    ...
    for (let x = 0; x < n; x ++) {
        for (let y = 0; y < n; y ++) {
            if (board[y][x]) {
                let this_square = [y, x]
                let result = getConflitosCount(y, x)
                let conflicts = result[1]
                totalConflicts[this_square] = []
                for (let i = 0; i < conflicts.length; i++) {
                    let conflict_square = conflicts[i]
...
function getConflitosCount(current_y, current_x) {
    conflictSquares = []
    count = 0;

    // Verifica apenas o quadrado à direita
...
// Diagonalmente para cima à direita
    var y = current_y - 1
    var x = current_x + 1 
    while (y >= 0 && x < n) {
        if (board[y][x]){
    ...
    // Diagonalmente para baixo à direita
    var y = current_y + 1
    var x = current_x + 1 
    while (y < n && x < n) {
        if (board[y][x]){
